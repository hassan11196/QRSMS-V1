...
class Campus(models.Model):
    ...
    contact_no = models.IntegerField(
        help_text='Contact Number of Campus', null=True, name='contact_no')
    contact_email = models.EmailField(
        help_text='Email of Campus', null=True, name='contact_email')
...
class Department(models.Model):
    ...
    department_hod = models.ForeignKey("actor.User", help_text="Current HOD of Department",
                                       name='department_hod', on_delete=models.SET_NULL, null=True)
    department_teachers = models.ManyToManyField(
        "teacher_portal.Teacher", related_name='teachers_in_department')
    department_students = models.ManyToManyField(
        "student_portal.Student", related_name='students_in_department')
    department_faculty = models.ManyToManyField(
        "faculty_portal.Faculty", related_name='faculty_in_department')
...
class Degree(models.Model):
    ...
    minimium_years_education = models.PositiveSmallIntegerField(
        null=True, name='minimium_years_education', help_text='minimium years of education required for admission in Degree')
    completion_year = models.PositiveSmallIntegerField(
        null=True, name='completion_year', help_text='Years of Education after completion of degree')
    duration = models.PositiveSmallIntegerField(
        null=True, name='duration', help_text='Duration of Degree Program')
    ...
    registrations_open = models.BooleanField(
        help_text='True if Course Registrations are Open, else False', default=False, blank=True, null=True)
    registration_semester = models.ForeignKey(
        'initial.Semester', on_delete=models.SET_NULL, blank=True, null=True)
...
class BatchSection(models.Model):
    ...
    section = models.CharField(
        max_length=10, help_text='Section Name Etc A,B,C etc')
    ...
    def fetch_all_students(cls):
        ...
        for student in s:
            s, created = cls.objects.get_or_create(
                section=student.admission_section, batch=student.batch)
            if created:
                print(s)
